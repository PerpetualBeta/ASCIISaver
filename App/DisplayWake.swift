...
/// (where this was written, and ASCII Saver copies it) decided whether to activate from two things only, how long since the last
...
///
/// The check cannot see ASCII Saver's own reflection, if it has one:
/// it runs only while no saver window is up, and
/// the camera is stopped whenever no saver window is up.
enum DisplayWake {
