...
/// has — so Settings can warn when ASCII Saver's own idle timeout is set at
...
/// for ASCII Saver's own idle timeout to lose a race against.
///
...
/// ASCII Saver's idle timeout has to beat is the soonest of *those*. A lock
...
enum SystemScreenLockSettings {
    ...
    /// unreadable) — nothing for ASCII Saver's own idle timeout to lose a
    /// race against at all.
