...
final class ASCIIRenderView: NSView {
    ...
    // MARK: - Inputs
    // Fed directly by CameraCaptureService now; was an mmap of the agent's buffer.
    ...
    /// Stop the display link and drop it.
    ///
    /// The output callback holds `self` **unretained** (passUnretained, as it
    /// must — a retained pointer would make the view immortal). Under
    /// legacyScreenSaver that was survivable: the host tore the whole process
    /// down. This app creates and destroys a render view per screen on every
    /// activation, so a link left running past deinit would call back into
    /// freed memory. Stopping it here, and in deinit as a backstop, closes
    /// that.
    func stopRendering() {
    ...
    /// Halt the render loop without tearing the view down — used while the
    /// system lock screen covers us, where there is no point burning CPU and
    /// camera on frames nobody can see.
    func pauseRendering() {
    ...
    private func tick() {
        ...
        if let shared = frameSource.readIfNew(),
